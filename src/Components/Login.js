...
componentDidMount(){
    fetch('/jwtt')
        .then(res => res.json())
        .then(jwtt => this.setState({jwtt}, () => console.log('fetched...', jwtt)));
}
    changeHandler(e){
this.setState({
    ...
    render() {
        ...
        if(this.state.count===2){
            return(
                alert("You have already taken two books...")
            )
        }
        return (
            <div>
                <h1 className="login">Login Panel</h1>
